...
def push(server, path, filename, serialization="yaml"):
    """
    Store a yaml coniguration to the server
    :param server: zk server
    :param path: path to store data
    :param filename: yaml file to upload
...
def pull(server, path, filename, serialization="yaml"):
    """
    pull a yaml configuration from the server
    :param server: zk server
    :param path: path to pull data from
    :param filename: optional file to save configuration
...
def move_copy(server, from_path, to_path, do_move=True, recursive=False):
    ...
    for p in server.get_children(from_path):
        if server[from_path + "/" + p]:
            server[to_path + "/" + p] = server[from_path + "/" + p]
        move_copy(server, from_path + "/" + p, to_path + "/" + p, do_move, recursive)

        if server.get_children(from_path + "/" + p) and do_move:
            server.delete(from_path + "/" + p)
    server.delete(from_path)
...
def dump_to_dir(server, path, recursive=False, indent=0):
    ...
    for p in server.get_children(path):
        if p == "zookeeper":
            continue
        dump_to_dir(server, path + "/" + p, recursive=True, indent=indent + 1)
...
def main():
    ...
    if args.command == "load":
        ...
        try:
            if args.recursive:
                print("\nhost:", server.hosts)
                print("-" * len("host: " + server.hosts))
            result = list_children(server, args.path, args.recursive)
            if result:
                print(result)
        except kazoo.exceptions.NoNodeError:
            print("Could not find " + args.path + " on server.")
            return 1
